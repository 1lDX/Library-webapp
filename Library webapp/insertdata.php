...
if (empty($_REQUEST['studentID']) || empty($_REQUEST['firstname']) || empty($_REQUEST['lastname']) || empty($_REQUEST['Return_Date']) || empty($_REQUEST['BookName'])) {
    echo "กรุณากรอกข้อมูลให้ครบทุกช่อง";
    exit;
}

// แปลงข้อมูลผู้ใช้ให้เป็นสตริงที่ปลอดภัย
$studentID = mysqli_escape_string($conn, $_REQUEST['studentID']);
